...
// https://www.npmjs.com/package/canvas-text-wrapper

export default function CanvasTextWrapper (canvas, text, options) {
  'use strict';
  ...
  let scale = 1;
  const devicePixelRatio = (typeof window !== 'undefined') ? window.devicePixelRatio : this.devicePixelRatio;
  ...
  const EL_WIDTH = (!opts.fitParent ? canvas.width : canvas.parentNode.clientWidth) / scale;
  const EL_HEIGHT = (!opts.fitParent ? canvas.height : canvas.parentNode.clientHeight) / scale;
  const MAX_TXT_WIDTH = EL_WIDTH - (opts.paddingX * 2);
  const MAX_TXT_HEIGHT = EL_HEIGHT - (opts.paddingY * 2);

  const fontSize = opts.font.match(/\d+(px|em|%)/g) ? +opts.font.match(/\d+(px|em|%)/g)[0].match(/\d+/g) : 18;
  ...
  function setLineHeight () {
    ...
    } else if (opts.lineHeight.toString().includes('px')) {
      lineHeight = parseInt(opts.lineHeight);
    } else if (opts.lineHeight.toString().includes('%')) {
